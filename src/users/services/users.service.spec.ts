...
describe('users service tests', () => {
  ...
  describe('createUser', () => {
    it('should create a new user', async () => {
      ...
      (roleRepo.findRolebyName as jest.Mock).mockResolvedValue('client');
  ...
  describe('changeRole', () => {
    it('should return a user with his role changed', async () => {
      const mockUser = {
        name: 'name',
        lastname: 'lastname',
        email: 'email',
      ...
      const user = await service.changeRole('userId', 'admin');

      expect(userRepo.findOne).toHaveBeenCalledWith('userId', {
        relations: ['role'],
      });
      expect(roleRepo.findOne).toHaveBeenCalledWith({ name: 'admin' });
      expect(userRepo.save as jest.Mock).toHaveBeenCalledWith({
        ...mockUser,
        role: mockRole,
      });
      expect(user).toStrictEqual({ ...mockUser, role: 'admin' });
    });
  });
});
